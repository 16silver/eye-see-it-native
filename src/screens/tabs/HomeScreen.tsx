...
import { StyleSheet, FlatList, TouchableOpacity, Alert, Platform, Modal, TextInput, View as RNView, StatusBar } from 'react-native';
...
export default function ListScreen() {
  ...
  const route = useRoute<TabScreenProps<'Home'>['route']>();
  ...
  const [searchType, setSearchType] = useState<'all' | 'artist' | 'name' | 'location'>('all');
  ...
  const isAppPhotoUri = (uri: string) => {
    const dir = RNFS.DocumentDirectoryPath + '/' + APP_PHOTO_DIR;
    return typeof uri === 'string' && (uri.includes(dir) || uri.includes(APP_PHOTO_DIR));
  };
  ...
  const handleAddPhoto = async (exhibitionId: string) => {
    ...
    const addFromNativeGallery = async () => {
      try {
        ...
        const assetUris = (result.assets || []).map(a => a.uri).filter(Boolean) as string[];
        const uris: string[] = [];
    ...
    const addFromNativeCamera = async () => {
      ...
      navigation.navigate('Camera' as never, { targetId: exhibitionId } as never);
    };
  ...
  const handleDatePress = (visitDate: string) => {
    // Album 스택 스크린으로 이동하면서 특정 날짜로 스크롤하도록 파라미터 전달
    navigation.navigate('Album' as never, { scrollToDate: visitDate } as never);
  };
    ...
    if (searchQuery.trim()) {
      ...
      list = list.filter((e) => {
        if (searchType === 'all') {
          ...
          return (e.location || '').toLowerCase().includes(q);
        }
        return true;
      });
    }
  ...
  return (
    <View style={styles.container}>
      <StatusBar barStyle="light-content" backgroundColor="#00D26A" />
      {/* 헤더 - 연도 칩, 즐겨찾기 토글, 메뉴 */}
      <RNView
        style={[
      ...
      >
        {isSearchMode ? (
          // 검색 모드 헤더
          <RNView style={styles.searchHeader}>
            {/* 드롭다운 버튼 */}
            <TouchableOpacity 
              style={styles.searchTypeButton}
              onPress={() => setShowSearchTypeDropdown(!showSearchTypeDropdown)}
              activeOpacity={0.7}
            >
              <Text style={styles.searchTypeText}>
                {searchType === 'all' ? '통합' : 
                 searchType === 'artist' ? '작가명' :
                 searchType === 'name' ? '전시명' : '장소'}
              </Text>
              <FeatherIcon name="chevron-down" size={16} color="#fff" />
            </TouchableOpacity>
          ...
          <RNView style={styles.header}>
            <RNView style={styles.headerLeft}>
              ...
              >
                <Text style={[styles.yearChipText, { backgroundColor: 'transparent' }]}>{selectedYear ?? new Date().getFullYear()}</Text>
              </TouchableOpacity>
            </RNView>
            <RNView style={styles.headerRight}>
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <FeatherIcon name="search" size={24} color="#ffffff" />
              </TouchableOpacity>
              <TouchableOpacity
              ...
              >
                <FeatherIcon name="menu" size={24} color="#ffffff" />
              </TouchableOpacity>
            </RNView>
          </RNView>
        )}
        ...
        {showSearchTypeDropdown && (
          <RNView style={styles.searchTypeDropdown}>
            {[
              { value: 'all' as const, label: '통합검색' },
              { value: 'artist' as const, label: '작가명' },
              { value: 'name' as const, label: '전시명' },
              ...
              >
                <Text style={[
                  ...
                  searchType === option.value && styles.dropdownItemTextActive
                ]}>{option.label}</Text>
              </TouchableOpacity>
            ))}
          </RNView>
      ...
      {!showDetailModal && !showGalleryModal && !showAddModal && (
        ...
        >
          <RNView style={styles.fabIcon}>
            <FontAwesomeIcon name="camera" size={26} color="#ffffff" />
          </RNView>
        </TouchableOpacity>
      )}
    </View>
