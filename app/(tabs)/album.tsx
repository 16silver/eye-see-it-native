...
import { StyleSheet, Text, View, TouchableOpacity, FlatList, Image, Alert, Modal, Dimensions, Platform } from 'react-native';
...
export default function AlbumScreen() {
  ...
  useEffect(() => {
    // 웹에서도 CameraView가 로드 가능한지 테스트 플래그 (Expo SDK 53에서 일부 브라우저만 지원)
    if (Platform.OS === 'web') {
      try {
        // 단순 존재 체크로 가용성 플래그 세팅
        if (CameraView) setCameraSupportsWeb(true);
      } catch (e) {
  ...
  const takePicture = async () => {
    try {
      ...
      await addPhotoUsingPolicy(photo.uri);
      ...
      {viewMode === 'all' ? (
        ...
        <View style={styles.exhibitionView}>
          <View style={styles.exhibitionHeader}>
            ...
            >
              <FontAwesome name="arrow-left" size={20} color={Colors[(colorScheme ?? 'light') as 'light' | 'dark'].tint} />
                ...
                <>
                  ...
                  <View style={styles.cameraControls}>
                    ...
                    <TouchableOpacity style={styles.cameraButton} onPress={takePicture} disabled={!isCameraReady}>
                      <Ionicons name="radio-button-on" size={48} color={isCameraReady ? '#fff' : 'rgba(255,255,255,0.5)'} />
                    </TouchableOpacity>
