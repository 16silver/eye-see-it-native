...
function ToggleSection({ title, children, previewText, previewLines = 0 }: { title: string; children: React.ReactNode; previewText?: string; previewLines?: number }) {
  ...
  return (
    <View style={{ marginBottom: 12 }}>
      <TouchableOpacity onPress={handleToggle} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 10 }}>
...
export default function ExhibitionDetailModal({
  ...
  const addMainPhotos = async () => {
    ...
    Alert.alert('사진 추가', '방법을 선택하세요', [
      {
        text: '촬영',
        onPress: () => router.push({ pathname: '/camera', params: { targetId: exhibition.id, section: 'photos' } }),
      },
      {
        text: '갤러리에서 선택',
        onPress: async () => {
    ...
    Alert.alert('사진 추가', '방법을 선택하세요', [
      {
        ...
        onPress: () => {
          ...
          router.push({ pathname: '/camera', params: { targetId: exhibition.id, section: sec } });
        }
      },
      {
        text: '갤러리에서 선택',
                ...
                {RNPlatform.OS === 'web' ? (
                  ...
                  <>
                    <TouchableOpacity
                      ...
                      activeOpacity={0.7}
                    >
                      <FontAwesome name="calendar" size={14} color="#0c4b1f" />
